...
//!
//! This module implements the static Huffman code used by HPACK for
//! compressing header field values.
...
static HUFFMAN_ENCODE: [HuffmanCode; 257] = [
    HuffmanCode {
        code: 0x1ff8,
        bits: 13,
    }, // 0
...
pub fn encoded_len(data: &[u8]) -> usize {
    let mut bits = 0usize;
    for &byte in data {
        bits += HUFFMAN_ENCODE[byte as usize].bits as usize;
    }
...
pub fn encode(data: &[u8], out: &mut Vec<u8>) {
    ...
    for &byte in data {
        let entry = &HUFFMAN_ENCODE[byte as usize];

        // Add the code to our bit buffer
        bits <<= entry.bits;
...
pub enum HuffmanError {
    ...
    InvalidCode,
    /// EOS symbol encountered in the middle of the string.
    EosEncountered,
    /// Padding is not all 1s or is too long.
    InvalidPadding,
}
...
impl std::fmt::Display for HuffmanError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            HuffmanError::EosEncountered => write!(f, "EOS symbol in Huffman string"),
...
enum HuffmanNode {
    ...
    /// Leaf node with symbol (0-256, where 256 = EOS).
    Leaf { sym: u16 },
}

/// Get or build the Huffman decode tree.
fn decode_tree() -> &'static [HuffmanNode] {
    static TREE: OnceLock<Vec<HuffmanNode>> = OnceLock::new();
    TREE.get_or_init(build_decode_tree)
}

/// Build the Huffman decode tree from the encoding table.
fn build_decode_tree() -> Vec<HuffmanNode> {
    // We need enough space for all internal nodes plus leaves.
    // Maximum depth is 30 bits, so we could have up to 2^30 nodes,
    // but in practice the tree is much smaller.
    let mut nodes = Vec::with_capacity(1024);

    // Root node
    nodes.push(HuffmanNode::Internal { left: 0, right: 0 });

    // Insert each symbol into the tree
    for (sym, entry) in HUFFMAN_ENCODE.iter().enumerate() {
        let mut node_idx = 0usize;

        // Walk through each bit of the code (MSB first)
        for bit_pos in (0..entry.bits).rev() {
            ...
            let (left, right) = match nodes[node_idx] {
                HuffmanNode::Internal { left, right } => (left, right),
                HuffmanNode::Leaf { .. } => panic!("Huffman code collision"),
            };
            ...
            if is_last {
                ...
                nodes.push(HuffmanNode::Leaf { sym: sym as u16 });
                // Update parent's child pointer
                if bit == 0 {
                    nodes[node_idx] = HuffmanNode::Internal {
                        left: leaf_idx,
                        right,
                    };
                } else {
...
///
/// Returns an error if the data is invalid (e.g., contains EOS symbol
/// or has padding that isn't all 1s).
pub fn decode(data: &[u8], out: &mut Vec<u8>) -> Result<(), HuffmanError> {
        ...
        for bit_pos in (0..8).rev() {
            ...
            match tree[node_idx as usize] {
                HuffmanNode::Internal { left, right } => {
                    ...
                    if let HuffmanNode::Leaf { sym } = tree[node_idx as usize] {
                        if sym == 256 {
                            return Err(HuffmanError::EosEncountered);
                        }
                        out.push(sym as u8);
                ...
                HuffmanNode::Leaf { .. } => {
                    // We should have moved back to root after emitting
                    unreachable!();
                }
            }
        }
    ...
    // Verify padding: must be at most 7 bits and must be all 1s (EOS prefix)
    if node_idx != 0 {
        ...
        // The padding should be a prefix of EOS (all 1s)
        // We can verify this by checking if following all 1-bits from current node
        // would lead toward EOS.
        let mut check_node = node_idx;
        for _ in 0..padding_bits {
            match tree[check_node as usize] {
                HuffmanNode::Internal { right, .. } => {
                    if right == 0 {
                        return Err(HuffmanError::InvalidPadding);
                    }
                    check_node = right;
...
mod tests {
    ...
    fn test_encode_simple() {
        ...
        // Verify encoded length is shorter than original
    ...
    fn test_encoded_len() {
        ...
        let len = encoded_len(b"a");
        let mut encoded = Vec::new();
        encode(b"a", &mut encoded);
        assert_eq!(len, encoded.len());
    }

    #[test]
    fn test_all_bytes_roundtrip() {
        // Test that all byte values can be encoded and decoded
